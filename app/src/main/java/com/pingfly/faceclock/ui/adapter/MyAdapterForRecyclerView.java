 ...
 */
public abstract class MyAdapterForRecyclerView<T> extends RecyclerView.Adapter<MyViewHolderForRecyclerView> {

    private Context mContext;
    private int mDefaultLayoutId = 0;
    private List<T> mData;
    private MyHeaderAndFooterAdapter mHeaderAndFooterAdapter;
     ...
     */
    public MyAdapterForRecyclerView(Context context, List<T> data) {
        mContext = context;
        mData = data;
    }
     ...
     */
    public MyAdapterForRecyclerView(Context context, List<T> data, int defaultLayoutId) {
        this(context, data);
    ...
    /**
     * 修改onCreateViewHolder方法，找到view里的子控件
     * @param parent
     * @param viewType
     * @return
     */
    @NonNull
    @Override
    public MyViewHolderForRecyclerView onCreateViewHolder(ViewGroup parent, int viewType) {
        MyViewHolderForRecyclerView holder = new MyViewHolderForRecyclerView(mContext, View.inflate(mContext, viewType, null));
        holder.setOnItemClickListener(mOnItemClickListener);
        holder.setOnItemLongClickListener(mOnItemLongClickListener);
        holder.setOnItemTouchListener(mOnItemTouchListener);

        // 新增对checkbox的listener
    ...
    /**
     * 修改onBindViewHolder方法，初始化数据，然后设置监听
     * @param holder
     * @param position
     */
    @Override
    public void onBindViewHolder(@NonNull MyViewHolderForRecyclerView holder, int position) {
    ...
    // convert()作用???
    public abstract void convert(MyViewHolderForRecyclerView helper, T item, int position);
    ...
    public MyHeaderAndFooterAdapter getHeaderAndFooterAdapter() {
        if (mHeaderAndFooterAdapter == null) {
            synchronized (MyHeaderAndFooterAdapter.class) {
                if (mHeaderAndFooterAdapter == null) {
                    mHeaderAndFooterAdapter = new MyHeaderAndFooterAdapter(this);
                }
            }
        }
